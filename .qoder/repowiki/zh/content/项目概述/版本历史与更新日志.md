...
### 版本历史与里程碑
- 1.1.4（2025.02.05）
  - 新增新手引导（Tour）：首次打开自动展示，Ant Design 风格高亮与气泡提示，覆盖收藏夹列表、设置默认收藏夹、关键词过滤、智能操作。
  - 主题更新：切换为多巴胺配色风格（紫色/粉色渐变主题）。
  - 代码风格统一：统一使用 cn 函数替代 classNames。
  - UI 优化：优化 keyword 组件边框颜色。
- 0.0.0（2024.12.02）
...
## 结论
本项目遵循 SemVer 2.0，采用自动化发布流程，版本迭代聚焦用户体验与 AI 能力增强。通过新手引导、主题统一与流式 AI 处理，显著提升了易用性与性能。未来版本将持续完善 AI 能力、优化交互体验并加强数据安全与隐私保护。
