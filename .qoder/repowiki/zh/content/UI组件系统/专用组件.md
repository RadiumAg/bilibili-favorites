...
## 更新摘要
**所做更改**
- 新增 Tourist 向导组件章节，详细介绍基于 Lucide React 图标的现代化向导系统
- 更新核心组件概览，将 Tourist 组件纳入专用组件体系
- 增加向导组件的架构分析和使用示例
- 扩展组件依赖关系图，包含新的向导组件
...
## 简介
本文件聚焦于项目中的五个专用组件：Keyword（关键词组件）、FavoriteTag（收藏标签组件）、FinishedAnimate（完成动画组件）、KeywordModeSelector（关键词模式选择器）以及 Tourist（向导组件）。它们共同服务于"B站收藏夹管理"场景，分别承担关键词输入与展示、默认收藏夹设置与高亮、批量处理完成后的视觉反馈、关键词提取模式的选择与切换，以及用户引导与功能演示。文档将从组件职责、属性配置、事件处理、业务逻辑、使用示例、集成方式、定制化选项与最佳实践等方面进行系统阐述，并给出可视化图示帮助理解。

## 项目结构
这些组件位于 src/components 和 src/popup/components 下，配套的业务逻辑通过 hooks 与全局状态 store 进行解耦，工具函数与算法封装在 utils 中，样式与类名合并工具位于 lib。
...
- [src/popup/components/tourist/index.tsx:1-293](file://src/popup/components/tourist/index.tsx#L1-L293)
...
- [src/popup/components/tourist/use-tourist.ts:1-84](file://src/popup/components/tourist/use-tourist.ts#L1-L84)
...
- Tourist：现代化向导组件，基于 Lucide React 图标提供四步引导，包括收藏夹选择、默认收藏夹设置、关键字过滤和智能操作功能演示。
...
## 架构总览
五个组件通过 hooks 与全局状态协同工作，形成"视图组件 + 业务逻辑 + 状态存储"的清晰分层。Keyword 与 FavoriteTag 负责输入与展示，FinishedAnimate 提供完成态反馈，KeywordModeSelector 控制关键词提取策略，Tourist 提供用户引导体验。
...
participant TR as "Tourist"
participant UTR as "use-tourist"
...
**图表来源**
- [src/popup/components/tourist/index.tsx:32-78](file://src/popup/components/tourist/index.tsx#L32-L78)
...
### FavoriteTag 组件
- 职责：展示收藏夹列表，支持点击切换当前活动收藏夹，长按设置默认收藏夹；加载中使用骨架屏占位。
- 关键属性
  - className: 可选的容器类名。
- 关键事件与行为
  - 点击：触发切换 activeKey。
...
### Tourist 向导组件
- 职责：提供现代化的用户引导体验，基于 Lucide React 图标系统展示四个关键功能步骤，包括收藏夹选择、默认收藏夹设置、关键字过滤和智能操作。
- 关键属性
  - ref: TouristRef 类型的引用，支持 resetTourist 方法重置向导。
- 关键事件与行为
  - 自动检测用户是否已完成向导，未完成则自动显示。
  - 支持步骤导航：上一步、下一步、跳过、完成。
  - 基于 SVG 遮罩高亮目标元素，提供沉浸式引导体验。
  - 使用渐变色进度条显示完成进度。
- 关键图标与步骤
  - 第一步：FolderHeart（收藏夹图标）- 收藏夹列表选择
  - 第二步：Star（星形图标）- 默认收藏夹设置
  - 第三步：Tag（标签图标）- 关键字过滤
  - 第四步：Gamepad2（游戏手柄图标）- 智能操作
- 依赖与数据流
  - 使用 use-tourist 管理向导状态和生命周期。
  - 依赖 useFavoriteData 获取收藏夹数据以决定是否显示向导。
  - 使用 Chrome Storage 本地存储向导完成状态。
- 交互流程

```mermaid
flowchart TD
Start(["页面加载"]) --> Check{"检查向导完成状态"}
Check --> |未完成| Show["显示向导界面"]
Check --> |已完成| End["跳过向导"]
Show --> Highlight["SVG遮罩高亮目标元素"]
Highlight --> Progress["显示进度条"]
Progress --> Navigate{"用户操作"}
Navigate --> |下一步| Next["进入下一步"]
Navigate --> |上一步| Prev["返回上一步"]
Navigate --> |跳过| Skip["标记完成并隐藏"]
Navigate --> |完成| Complete["标记完成并隐藏"]
Next --> Highlight
Prev --> Highlight
Skip --> End
Complete --> End
...
- [src/popup/components/tourist/use-tourist.ts:60-71](file://src/popup/components/tourist/use-tourist.ts#L60-L71)

**章节来源**
- [src/popup/components/tourist/index.tsx:20-50](file://src/popup/components/tourist/index.tsx#L20-L50)
- [src/popup/components/tourist/index.tsx:67-293](file://src/popup/components/tourist/index.tsx#L67-L293)
...
TR["Tourist"] --> UTR["use-tourist"]
UEK --> GDT["global-data"]
USDF --> GDT
UCK --> GDT
UCK --> KE["keyword-extractor"]
UTR --> CSG["Chrome Storage"]
TR --> UTIL["utils.cn"]
TR --> AH["ahooks"]
KW --> UTIL
FT --> UTIL
...
- Tourist
  - 使用 useMemoizedFn 优化事件处理器性能。
  - SVG 遮罩渲染在现代浏览器中性能良好。
  - 响应式布局自动适配窗口大小变化。
  - Chrome Storage 访问采用异步方式，不影响主界面性能。
...
- 向导不显示
  - 检查收藏夹数据是否加载完成。
  - 确认 Chrome Storage 中的完成状态键值是否正确。
  - 验证目标元素选择器是否正确匹配 DOM 元素。
- 向导定位错误
  - 检查目标元素是否存在且可见。
  - 确认 CSS 样式是否影响元素定位。
  - 验证窗口尺寸变化事件是否正常响应。
...
- [src/components/finished-animate/index.tsx:19-106](file://src/components/finished-animate/index.tsx#L19-L106)
- [src/components/keyword-mode-selector/index.tsx:17-46](file://src/components/keyword-mode-selector/index.tsx#L17-L46)
- [src/popup/components/tourist/index.tsx:86-148](file://src/popup/components/tourist/index.tsx#L86-L148)
- [src/popup/components/tourist/use-tourist.ts:60-71](file://src/popup/components/tourist/use-tourist.ts#L60-L71)

## 结论
这五个专用组件围绕"B站收藏夹管理"的核心业务场景构建，通过清晰的职责划分与稳定的 hooks/状态解耦，实现了关键词输入与管理、收藏夹切换与默认设置、批量处理完成反馈、关键词提取模式控制以及现代化用户引导体验。Tourist 向导组件的引入显著提升了用户体验，基于 Lucide React 图标的现代化设计使其在视觉上更加统一和专业。这些组件既可独立使用，也可组合在更高层的页面中协同工作，具备良好的扩展性与可维护性。
...
- [src/popup/components/tourist/index.tsx:63-65](file://src/popup/components/tourist/index.tsx#L63-L65)
...
- 在页面中引入组件并传入所需属性
  ...
  - Tourist：在主界面底部渲染，通过 ref 引用控制重置功能。
- 与 hooks 集成
  ...
  - Tourist：通过 use-tourist 获取 isVisible、currentStep、nextStep、prevStep 等状态和方法。
- 向导集成示例
  - 在 Popup 组件中引入 Tourist，并通过 ref 引用提供重置功能。
  - 使用 data-tour 属性标记需要高亮的目标元素。
  - 确保向导组件在所有目标元素加载完成后显示。
...
- [src/popup/components/tourist/index.tsx:67-78](file://src/popup/components/tourist/index.tsx#L67-L78)
...
- 定制化选项
  ...
  - Tourist：可扩展步骤数量、图标类型、引导文案、目标元素选择器。
- 最佳实践
  ...
  - 向导组件应考虑多语言支持和无障碍访问。
  - 向导步骤应与实际功能保持同步，避免过时的引导信息。
  - 使用 data-tour 属性标记目标元素，确保向导的准确性。
  - 向导状态应持久化存储，避免重复引导用户。