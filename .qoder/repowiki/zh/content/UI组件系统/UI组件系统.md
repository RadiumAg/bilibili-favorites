...
## 更新摘要
**所做更改**
- 新增Checkbox和Switch组件的详细文档说明
- 增强Toaster组件功能说明，包含展开错误详情功能
- 更新按钮样式和间距的统一规范
- 完善字体系统的使用指南
- 更新组件架构图以反映新增组件
...
**更新**：本次更新重点介绍了新增的Checkbox和Switch组件，这些基于Radix UI的交互组件为表单操作提供了更好的可访问性和用户体验。同时增强了Toaster组件的功能说明，支持展开详细的错误信息展示。按钮样式和间距的统一规范确保了组件间的一致性。字体系统的优化提升了文本显示质量和可读性。
...
### Toaster 组件交互流程
**更新**：Toaster 组件支持展开详细的错误信息，提供更好的错误诊断体验。
...
### 组合模式与复用策略
- 组合模式：Form 与 FormField/FormControl/Label/Message 协同；Card 分区组合；Select 内部子组件组合；Checkbox/Switch 与 Form 配合。
- 复用策略：通过 Variants（如 Button/Label/Badge）与 className 扩展实现跨页面复用；cn 工具保证样式叠加稳定。
- 事件处理：Button/Select/Popover/Checkbox/Switch 等组件透传原生事件，同时提供回调钩子；表单组件通过 react-hook-form 提供统一状态管理。
...
## 依赖关系分析
- 组件间耦合：基础组件低耦合，复合组件通过基础组件组合；Form 依赖 react-hook-form 与 Radix UI；Select/Popover/Progress/Checkbox/Switch/Toaster 等依赖 Radix UI。
...
- **更新**：Toaster 组件优化了消息队列管理，限制同时显示的消息数量，提升性能表现。

## 故障排查指南
- 可访问性问题
  - 表单错误未被读屏识别：检查 FormLabel 是否绑定到对应控件 ID，FormMessage 是否存在且被 aria-describedby 包含。
  - 键盘不可达：确认 Select/Popover/Tabs/Checkbox/Switch/Toaster 使用了 Radix UI 触发器与键盘导航。
- 样式冲突
...
**更新**：本次更新显著增强了UI组件系统的功能完整性。新增的Checkbox和Switch组件基于Radix UI提供了更好的可访问性和用户体验。Toaster组件的展开错误详情功能提升了错误诊断能力。按钮样式和间距的统一规范确保了组件间的一致性。字体系统的优化改善了文本显示质量。这些改进共同提升了整个UI组件系统的专业性和易用性。
...
- **更新**：Checkbox和Switch组件在移动设备上的触摸友好性得到优化，提供更好的触控体验。
...
- **更新**：Radix UI 组件在不同浏览器中的兼容性得到进一步优化，确保跨平台一致性。
...
- **更新**：Checkbox和Switch组件提供完整的键盘操作支持和屏幕阅读器识别，确保残障用户的无障碍使用。
...
- **更新**：Toaster组件的展开按钮提供清晰的中文 aria-label，支持屏幕阅读器正确播报。
...
- **更新**：Button组件的样式系统得到统一优化，确保所有变体在不同尺寸下的一致性表现。
