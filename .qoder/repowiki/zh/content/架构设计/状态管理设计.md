...
## 更新摘要
**变更内容**
- 存储中间件顺序调整：从嵌套顺序改为链式调用，确保setState回调的正确执行顺序
- IndexedDB自动迁移功能增强：增加从Chrome存储到IndexedDB的一次性数据迁移机制
- 数据库常量导出：`DB_NAME`和`DB_VERSION`常量现在可被外部模块访问
- WebDAV同步服务改进：增强对`keyword`字段的同步支持，提供更完整的跨设备数据同步
- 状态持久化策略优化：区分不同类型数据的最佳存储位置，提升系统整体性能
...
- [src/store/chorme-storage-middleware.ts:1-81](file://src/store/chorme-storage-middleware.ts#L1-L81)
- [src/store/indexeddb-storage-middleware.ts:1-94](file://src/store/indexeddb-storage-middleware.ts#L1-L94)
- [src/utils/sync-service.ts:1-308](file://src/utils/sync-service.ts#L1-L308)
...
- [src/utils/data-context.ts:5-42](file://src/utils/data-context.ts#L5-L42)
...
- [src/options/components/setting/index.tsx:14-58](file://src/options/components/setting/index.tsx#L14-L58)
...
- [src/store/indexeddb-storage-middleware.ts:12-84](file://src/store/indexeddb-storage-middleware.ts#L12-L84)
...
- **自动迁移功能**：支持从Chrome存储到IndexedDB的一次性数据迁移，确保数据完整性

```mermaid
flowchart TD
Start(["中间件初始化"]) --> Hydrate["从 IndexedDB tag-storage 读取 keyword"]
Hydrate --> Migration{"检查迁移需求"}
Migration --> |存在旧数据| Migrate["从 chrome.storage.local 迁移"]
Migration --> |无旧数据| Skip["跳过迁移"]
Migrate --> Clean["清理旧存储位置"]
Clean --> Merge["合并到初始状态"]
Skip --> Merge
Merge --> Ready["store 就绪"]
...
- **自动迁移**：支持从旧存储位置到新存储位置的数据迁移，确保用户数据无缝升级
...
- **相关实现文件路径**
  ...
  - [IndexedDB存储中间件:12-84](file://src/store/indexeddb-storage-middleware.ts#L12-L84)
  - [数据上下文类型:5-42](file://src/utils/data-context.ts#L5-L42)
  ...
  - [设置页面组件:14-58](file://src/options/components/setting/index.tsx#L14-L58)
