...
- [src/utils/batch-process.ts](file://src/utils/batch-process.ts)
...
## 更新摘要
**所做更改**
- 更新了AI移动功能章节，反映批量处理、JSON解析优化和进度跟踪的重大改进
- 新增了批量处理工具函数的详细说明
- 增强了JSON解析器的功能描述，包括残留缓冲区处理
- 完善了AI移动的用户界面和状态管理机制
- 添加了新的测试用例和验证机制
...
subgraph "工具与状态"
BatchProcess["utils/batch-process.ts"]
ParseJSON["utils/parse-ai-json.ts"]
StreamParser["hooks/use-create-keyword-by-ai/ai-stream-parser.ts"]
API["utils/api.ts"]
...
BatchProcess --> HookAIMove
ParseJSON --> HookAIMove
StreamParser --> HookCreateKW
```
...
- [src/popup/components/ai-move/use-ai-move.tsx:1-462](file://src/popup/components/ai-move/use-ai-move.tsx#L1-L462)
...
- [src/utils/batch-process.ts:1-24](file://src/utils/batch-process.ts#L1-L24)
- [src/utils/parse-ai-json.ts:1-65](file://src/utils/parse-ai-json.ts#L1-L65)
...
- 批量处理
  - 分页拉取默认收藏夹全部视频，使用批量处理工具函数进行分批处理。
  - 每批1000个视频，通过流式接口逐步拼接 AI 返回的 JSON。
  - 解析后进入移动阶段，支持进度跟踪和实时结果显示。
- 用户确认流程
  - 首次点击弹出"Token 消耗提醒"，二次点击才真正执行。
  - 支持取消：中断流与移动队列，清理资源。
- 进度跟踪与状态管理
  - 实时显示处理进度（当前/总数）、当前处理的视频标题。
  - 提供已完成结果列表，支持滚动查看详细状态。
  - 支持取消操作，中断流式读取和后续移动调用。

**更新** 重大增强包括批量处理优化、JSON解析改进和进度跟踪完善

```mermaid
sequenceDiagram
participant U as "用户"
participant AIM as "AI 移动 Hook"
participant BP as "批量处理工具"
participant API as "API 封装"
participant BG as "后台消息通道"
U->>AIM : 点击"AI 整理"
AIM->>AIM : 弹出确认提示
...
AIM->>API : 获取默认收藏夹全部视频
API-->>AIM : 返回视频列表
AIM->>BP : 调用批量处理(每批1000个)
loop 分批处理
BP->>API : 发送 AI 分析请求流式
API->>BG : 建立长连接并转发
BG-->>API : 流式返回片段
API-->>AIM : 解析 JSON 并映射目标收藏夹
AIM->>AIM : 更新进度跟踪(当前/总数/标题)
AIM->>AIM : 处理结果(成功/失败/跳过/兜底)
AIM->>BG : 发送移动消息
BG->>API : 转发移动请求
API-->>BG : 返回结果
BG-->>AIM : 成功/失败
...
- [src/popup/components/ai-move/use-ai-move.tsx:176-260](file://src/popup/components/ai-move/use-ai-move.tsx#L176-L260)
- [src/popup/components/ai-move/use-ai-move.tsx:90-169](file://src/popup/components/ai-move/use-ai-move.tsx#L90-L169)
- [src/utils/api.ts:257-279](file://src/utils/api.ts#L257-L279)
- [src/utils/batch-process.ts:13-23](file://src/utils/batch-process.ts#L13-L23)
- [src/utils/parse-ai-json.ts:20-64](file://src/utils/parse-ai-json.ts#L20-L64)
- [src/utils/tab.ts:65-82](file://src/utils/tab.ts#L65-L82)

**章节来源**
- [src/popup/components/ai-move/index.tsx:8-59](file://src/popup/components/ai-move/index.tsx#L8-L59)
- [src/popup/components/ai-move/use-ai-move.tsx:23-462](file://src/popup/components/ai-move/use-ai-move.tsx#L23-L462)
- [src/utils/api.ts:257-279](file://src/utils/api.ts#L257-L279)
- [src/utils/batch-process.ts:1-24](file://src/utils/batch-process.ts#L1-L24)
- [src/utils/parse-ai-json.ts:1-65](file://src/utils/parse-ai-json.ts#L1-L65)
...
### 批量处理工具函数
- 批量处理机制
  - 支持自定义批次大小（maxSize），默认1000个视频一批。
  - 串行执行每个批次的处理回调，确保请求有序性。
  - 空数组时不调用回调，避免不必要的处理。
- 分批策略
  - 使用 slice 方法按批次大小分割视频数组。
  - 每批处理完成后等待异步回调完成再继续下一批。
- 错误处理
  - 回调函数抛错时，整个批量处理过程会抛出相同错误。
  - 保证错误传播的一致性，便于上层处理。

**新增** 专门的批量处理工具函数，支持大规模视频整理的性能优化
...
### JSON解析优化
- 增量解析机制
  - 支持流式数据的增量解析，逐字符检测花括号配对。
  - 使用缓冲区累积数据，提取完整JSON对象后立即处理。
- 容错处理
  - 支持Markdown代码块包裹的JSON格式（```json ... ```）。
  - 使用jsonrepair库修复格式异常的JSON字符串。
- 残留数据处理
  - 处理完成后检查缓冲区剩余数据，尝试解析最后的JSON片段。
  - 对解析失败的残留数据进行警告记录，避免数据丢失。

**更新** JSON解析器进行了重大优化，支持更复杂的流式处理场景
...
- 适配器模式
  - 支持多种AI模型的流式响应格式解析。
  - 提供SparkStreamAdapter和OpenAIStreamAdapter两种实现。
- 内容提取
  - 从SSE流数据中提取实际的文本内容片段。
  - 支持不同模型的响应结构差异。
- 缓冲区管理
...
API --> Msg["message.ts"]
API --> Tab["tab.ts"]
AIMoveHook --> BatchProcess["batch-process.ts"]
AIMoveHook --> ParseJSON["parse-ai-json.ts"]
AIMoveHook --> StreamParser["ai-stream-parser.ts"]
```
...
- 批量处理优化
  - 使用1000个视频的批次大小，在性能和内存使用间取得平衡。
  - 串行处理确保请求有序性，避免并发冲突。
- UI 响应
  - 长耗时操作使用遮罩层与动画提示，提升用户体验；完成动画在结束后自动清理状态。
- JSON解析优化
  - 增量解析减少内存占用，支持大数据量的流式处理。
  - 残留缓冲区处理确保数据完整性。

**更新** 新增批量处理和JSON解析优化的性能考量
...
- "AI 返回格式错误"
  - 现象：JSON解析失败或格式异常。
  - 排查：检查AI模型返回格式，确认是否为标准JSON结构。
- "批量处理中断"
  - 现象：大批量视频处理过程中断。
  - 排查：检查网络连接稳定性，适当减小批次大小。

**更新** 新增AI格式错误和批量处理中断的故障排查指南
...
**更新** 重大功能增强包括批量处理优化、JSON解析改进和进度跟踪完善，显著提升了大规模视频整理的性能和可靠性。
...
- 最佳实践
  ...
  - 大批量视频整理时，系统会自动进行分批处理，无需手动干预。
- 常见问题
  - 登录态失效：确保 B 站标签页处于活动状态，或重新登录。
  - AI 返回格式错误：检查提示词构建与流式解析逻辑，必要时切换到自定义模型。
  - 移动过快被限流：适当增加间隔或减少并发。
  - 批量处理超时：检查网络连接稳定性，适当减小批次大小。
  - JSON解析失败：确认AI模型返回标准JSON格式，检查格式修复功能。