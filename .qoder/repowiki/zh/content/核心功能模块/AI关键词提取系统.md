...
## 更新摘要
**变更内容**
- 新增AIGate AI适配器支持，提供免费的大模型服务
- 集成AIGate免费额度到关键词提取和AI移动功能
- 修复use-set-default-fav钩子bug，提升系统稳定性
- 增强配置管理模式，支持免费额度和自定义配置
- 完善配额检查和管理功能
...
- **流式数据适配**：支持OpenAI、星火大模型和AIGate免费服务的不同响应格式
...
AIGate免费适配器是新增的核心功能，提供免费的大模型服务：

- **免费额度**：集成AIGate免费大模型，无需API Key即可使用
- **自动切换**：当自定义配置不可用时自动切换到免费服务
- **配额管理**：提供实时配额检查和使用情况监控
- **稳定可靠**：经过优化的适配器实现，提升系统稳定性
...
AIGate免费适配器是系统的重要新增功能，提供了完全免费的大模型服务：

```mermaid
classDiagram
class AIGateAdapter {
+callAIGateAI(messages) Promise
+parse(chunk) string
+supportsFreeQuota() boolean
}
class AIGateAPI {
...
+rpm : QuotaInfo
+checkQuota() void
+updateQuota(info) void
}
AIGateAdapter --> AIGateAPI : uses
AIGateAdapter --> QuotaManager : monitors
```

**图表来源**
- [api.ts:265-277](file://src/utils/api.ts#L265-L277)
- [free-quota-panel.tsx:12-59](file://src/options/components/setting/components/free-quota-panel.tsx#L12-L59)

#### AIGate免费额度使用流程

系统通过以下流程集成AIGate免费服务：

```mermaid
sequenceDiagram
participant User as 用户
participant Hook as 关键词创建Hook
participant API as AIGate API
participant Store as 状态存储
User->>Hook : 选择AIGate免费模式
Hook->>API : checkQuota()
API->>API : 检查配额状态
API-->>Hook : 返回配额信息
Hook->>API : callAIGateAI(messages)
API->>API : 发送AI请求
API-->>Hook : 返回流式响应
Hook->>Hook : 解析流式数据
Hook->>Store : 更新关键词
...
**图表来源**
- [use-create-keyword/index.tsx:129-133](file://src/hooks/use-create-keyword/index.tsx#L129-L133)
- [use-ai-move.tsx:108-112](file://src/popup/components/ai-move/use-ai-move.tsx#L108-L112)

#### 配额管理系统

AIGate免费适配器包含了完整的配额管理功能：

- **实时配额检查**：支持每日、每月和RPM（每分钟）配额检查
- **可视化显示**：通过进度条和状态指示器展示配额使用情况
- **自动更新**：配额信息会自动更新并显示最后检查时间
- **超时处理**：10秒超时机制确保用户体验

**章节来源**
- [api.ts:265-277](file://src/utils/api.ts#L265-L277)
- [free-quota-panel.tsx:1-67](file://src/options/components/setting/components/free-quota-panel.tsx#L1-L67)
- [quota-card.tsx:1-199](file://src/options/components/setting/components/quota-card.tsx#L1-L199)
...
AIStreamAdapter <|.. AIGateStreamAdapter
...
ExtractContent --> |AIGate| GetContent
