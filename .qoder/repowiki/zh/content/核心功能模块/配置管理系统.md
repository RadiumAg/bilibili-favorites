...
## 更新摘要
**所做更改**
- 新增CustomModeSwitch组件替代原有AI切换开关，提供更直观的配置模式切换体验
...
**更新** 引入了全新的CustomModeSwitch组件，提供更直观的配置模式切换体验，支持"使用内置免费 AI"的开关式操作。
...
Setting["Setting 组件"]
ConfigMode["配置模式选择器"]
CustomModeSwitch["CustomModeSwitch 组件"]
CustomForm["自定义配置表单"]
QuotaCard["配额卡片"]
FreePanel["免费配额面板"]
end
...
- **CustomModeSwitch组件**：新增的开关式组件，提供直观的"使用内置免费 AI"切换功能，替代原有的复杂按钮组设计。
...
**更新** 新增CustomModeSwitch组件提供更直观的配置模式切换体验，支持开关式操作和实时状态反馈。
...
participant CustomModeSwitch as "CustomModeSwitch"
participant ConfigMode as "ConfigModeSelector"
...
User->>CustomModeSwitch : 切换"使用内置免费 AI"
CustomModeSwitch->>Store : 更新configMode状态
User->>ConfigMode : 选择配置模式
ConfigMode->>Store : 更新configMode状态
Options->>Form : 初始化表单
Form->>Schema : 实时验证数据
Schema-->>Form : 返回验证结果
...
### CustomModeSwitch组件
- **新增功能**：提供直观的开关式配置模式切换
- **交互设计**：使用Switch组件配合Label标签，提供"使用内置免费 AI"的开关操作
- **状态管理**：通过useGlobalConfig钩子访问全局状态，实时反映当前配置模式
- **事件处理**：onCheckedChange回调函数根据开关状态切换configMode值
- **用户体验**：相比原有的按钮组，开关式设计更加直观和高效
...
+使用Switch组件提供开关式操作
+绑定Label标签增强可访问性
+通过useGlobalConfig访问全局状态
+监听onCheckedChange事件
+根据开关状态更新configMode
}
class ConfigModeSelector {
+包含CustomModeSwitch组件
+提供传统按钮组选择
+支持两种配置模式切换
}
CustomModeSwitch --> ConfigModeSelector : "被嵌套使用"
```

**图表来源**
- [src/options/components/setting/components/custom-mode-switch.tsx:1-35](file://src/options/components/setting/components/custom-mode-switch.tsx#L1-L35)
- [src/options/components/setting/components/config-mode-selector.tsx:1-46](file://src/options/components/setting/components/config-mode-selector.tsx#L1-L46)
...
- 组件耦合：
  ...
  - 配额卡片依赖背景脚本的消息枚举
  - 自定义配置表单依赖适配器工具与默认参数
  - **CustomModeSwitch组件依赖ConfigModeSelector组件**
- 外部依赖：
...
**更新** 新增了CustomModeSwitch组件及其依赖关系，提供了更直观的配置模式切换体验。
...
ConfigMode["ConfigModeSelector"] --> CustomModeSwitch["CustomModeSwitch"]
CustomModeSwitch --> Store
Quota --> BG["背景脚本"]
BG --> Msg["消息枚举"]
...
- **组件优化**：CustomModeSwitch组件使用useShallow优化状态订阅，减少不必要的重新渲染

**更新** 新的表单系统使用React Hook Form，提供了更好的性能和用户体验，CustomModeSwitch组件也进行了性能优化。
...
- 表单验证错误
  ...
  - 参考路径：[src/options/components/setting/types.ts:28-64](file://src/options/components/setting/types.ts#L28-L64)
- **CustomModeSwitch组件问题**
  - 现象：开关无法正常切换配置模式
  - 排查：检查useGlobalConfig钩子状态更新、Switch组件事件绑定
...
## 结论
本配置管理系统通过清晰的三层架构实现了灵活的AI服务配置与管理。自定义配置满足多样化服务商接入需求，免费额度模式降低使用门槛，配额管理提供透明的用量监控。结合Chrome Storage持久化与Port消息机制，系统在易用性与可靠性之间取得平衡。新的React Hook Form和Zod验证框架进一步提升了表单管理能力和用户体验。**新增的CustomModeSwitch组件提供了更直观的配置模式切换体验，简化了用户操作流程**。建议后续扩展用户偏好设置与云端同步能力，进一步提升用户体验。

**更新** 重构后的系统提供了更强大、更可靠的配置管理能力，特别是在表单验证和用户体验方面有了显著改进，CustomModeSwitch组件的引入进一步优化了用户界面交互。

## 附录

### 配置指南
- 首次设置
  - 打开选项页，使用CustomModeSwitch组件切换到"使用内置免费 AI"
  - 或选择"自定义配置"模式填写API Key
  - 填写相应字段并点击保存
  - 查看配额卡片确认配置生效
- 高级配置
  ...
  - 如需代理，填写"Base URL"
  - 定期检查配额，避免超出限制
  - 使用CustomModeSwitch快速在免费模式和自定义模式间切换
- 故障排除
  - 网络问题：检查AIGate服务状态与网络连通性
  - 配额不足：等待次日重置或升级套餐
  - 配置丢失：确认Chrome Storage权限与中间件工作正常
  - 表单验证：检查Zod验证规则和React Hook Form配置
  - **CustomModeSwitch问题**：检查开关状态同步和全局状态更新

**更新** 新增了CustomModeSwitch组件的使用指导和故障排除方法。
...
- 使用AIGate免费额度
  ...
  - 模型：选择免费模型名称
  - 配额检查：点击"检查配额"查看剩余次数
  - **使用CustomModeSwitch**：开启"使用内置免费 AI"开关
...
- **善用CustomModeSwitch组件**：通过开关式操作快速切换配置模式，提升工作效率

**更新** 新增了关于新表单系统和CustomModeSwitch组件的最佳实践建议。