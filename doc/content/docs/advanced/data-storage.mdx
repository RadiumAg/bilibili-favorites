...
| 键 | 含义 |
|-----|-----|
| `aiConfig.configMode` | `custom`（旧版本 `free` 会在升级时迁移为 `custom`） |
| `aiConfig.adapter` | `openai` / `qianwen` / `kimi` / `gml` / `custom` / `spark` |
...
<Callout type="info">
  Chrome Storage 会随 Chrome Sync（若已启用）同步到其他登录了相同账号的设备。若你不希望 API Key 被同步，请关闭 Chrome 账号同步，并避免在共享浏览器配置中保存凭据。
</Callout>
